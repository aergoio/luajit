...
static void close_state(lua_State *L)
{
    ...
    map_deinit(&((GG_State *)L)->gas_info);
...
LUA_API gas_info_t *get_gas_info(lua_State *L)
{
    GG_State *gg = L2GG(L);
    map_int_t *gi = &gg->gas_info;
    gas_info_t *result = (gas_info_t *)malloc(sizeof(gas_info_t));
    int max = result->n = gi->base.nnodes;
    int i = 0;
    const char *key = NULL;
    map_iter_t iter = map_iter(gi);

    result->keys = (char **)calloc(max, sizeof(char *));
    result->values = (int *)calloc(max, sizeof(int));

    while ((key = map_next(gi, &iter)))
    {
        result->keys[i] = strdup(key);
        result->values[i] = *(int *)map_get(gi, key);
        ++i;
    }
...
LUA_API void free_gas_info(gas_info_t *gi)
{
    int i;
    for (i = 0; i < gi->n; ++i)
    {
