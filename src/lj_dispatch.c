...
#endif
    /* In Aergo, whether VM uses gas lock or not is determined after GG_State is
     * created. So the mutex must be initialized unconditionally.
     */
    GG->use_gas_lock = 1;
    init_gas_lock(GG);
    acquire_gas_lock(GG);
    ...
    { /* Mode changed? */
        ...
        if ((oldmode ^ mode) & (DISPMODE_PROF | DISPMODE_REC | DISPMODE_INS))
        {
            ...
            else
            {
                ...
                ASMFunction f = (mode & DISPMODE_PROF) ? lj_vm_profhook : (mode & DISPMODE_REC) ? lj_vm_record
                                                                                                : lj_vm_inshook;
                uint32_t i;
                for (i = 0; i < GG_LEN_SDISP; i++)
                    disp[i] = f;
...
int luaJIT_setmode(lua_State *L, int idx, int mode)
{
    ...
    {
        cTValue *tv = idx == 0 ? frame_prev(L->base - 1) - LJ_FR2 : idx > 0 ? L->base + (idx - 1)
                                                                            : L->top + idx;
        GCproto *pt;
